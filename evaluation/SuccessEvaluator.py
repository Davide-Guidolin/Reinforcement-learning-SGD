...
class ObjectiveSuccessEvaluator(Evaluator):
    ...
    def __init__(self, domainString):
        ...
        self.action_slot_successReward_supervised = 10
        self.action_only_successReward_supervised = 5
        self.action_wrong_penalty_supervised = 10
        if Settings.config.has_option("exec_config","actionslotsuccessreward"):
            self.action_slot_successReward_supervised = Settings.config.getint("exec_config","actionslotsuccessreward")
        if Settings.config.has_option("exec_config","actiononlysuccessreward"):
            self.action_only_successReward_supervised = Settings.config.getint("exec_config","actiononlysuccessreward")
        if Settings.config.has_option("exec_config","actionwrongpenalty"):
            self.action_wrong_penalty_supervised = Settings.config.getint("exec_config","actionwrongpenalty")
    ...
    def _getTurnReward(self, turnInfo):
        ...
        if turnInfo is not None and isinstance(turnInfo, dict):
            ...
            # supervised
            if 'supervised' in turnInfo and 'sys_act' in turnInfo:
                if turnInfo['supervised']:
                    if 'real_sys_act' in turnInfo:
                        if turnInfo['real_sys_act']:
                            reward += self.getSupervisedReward(turnInfo)
                            logger.dial("REAL SYS ACT: "+str(turnInfo['real_sys_act']))
                            logger.dial("SUP. REWARD = "+str(reward))

        return reward

    def getSupervisedReward(self, turnInfo):
        reward = 0
        ...
        real_actions = []
        for action in turnInfo['real_sys_act']:
            ...
            real_actions.append((real_act, real_slot_name))

        action_correct = False
        slot_correct = False
        for test_act in real_actions:
        ...
        if action_correct:
            if slot_correct:
                reward += self.action_slot_successReward_supervised
            else:
                reward += self.action_only_successReward_supervised
        else:
            reward -= self.action_wrong_penalty_supervised
