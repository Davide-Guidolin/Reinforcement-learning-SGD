...
def _ParseAct(raw_act_text, user=True):
    ...
    if raw_act['act'] == "request" or raw_act['act'] == "confreq":
    ...
    elif (raw_act['act'] in ['negate','repeat','affirm','bye','restart','reqalts','hello','silence','thankyou','ack','help','canthear','reqmore', 'notify']):
        ...
        main_act_type = 'inform'
    elif (raw_act['act'] not in ['inform','deny','confirm','select','null', 'badact', 'offer']):
    ...
    if raw_act['act'] == "offer" and not user:
        final_dialog_act.append( {
            'act': 'offer',
            'slots': [[slot, value] for slot, value in raw_act['slots']]
            })
