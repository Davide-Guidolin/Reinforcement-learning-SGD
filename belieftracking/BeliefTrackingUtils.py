...
def _transformAct(act, valueTrans,ontology=None, user=True):
    ...
    for i in range(0,len(act)): 
        for j in range(0, len(act[i]["slots"])):    # if valuetrans replace slots in valuetrans
    ...
    if ontology != None:
        ...
        for a in act:
            ...
            for slot,value in a["slots"]:
                keep = True
                if slot not in ["slot","this"] and (slot not in ontology["informable"]) :   # keep slots that are in informable (user, system) or requestable (system)
