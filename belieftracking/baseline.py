...
#                 else:
            ...
            elif 'denied-goals' in track: # if in denied goals all values to 0, dontcare to 1
                if slot in track['denied-goals']:
                    infom_slot_vals = Ontology.global_ontology.get_informable_slot_values(self.domainString,slot)
                    belief[slot] = dict.fromkeys(infom_slot_vals+['dontcare'], 0.0)
                    belief[slot]['dontcare'] = 1.0
                    belief[slot]['**NONE**'] = 1.0 - sum(belief[slot].values())
                else:
        ...
        if Settings.config.has_section('policy_' + self.domainString):
            policytype = Settings.config.get('policy_' + self.domainString, 'policytype')
        else:
            policytype = Settings.config.get('policy', 'policytype')
        
        if policytype != 'gp':
            if track['sysAct'] == prev_belief['beliefs']['lastSysAct']:
                belief['sameSysAct'] = 1 #prev_belief['beliefs']['sameSysAct'] + 1
                belief['lastSysAct'] = prev_belief['beliefs']['lastSysAct']
            else:
                belief['sameSysAct'] = 0
                belief['lastSysAct'] = track['sysAct']

            if track['userAct'] == prev_belief['beliefs']['lastUserAct']:
                belief['sameUserAct'] = 1 #prev_belief['beliefs']['sameUserAct'] + 1
                belief['lastUserAct'] = prev_belief['beliefs']['lastUserAct']
            else:
                belief['sameUserAct'] = 0
                belief['lastUserAct'] = track['userAct']

            belief['uactConfirm'] = 0
            belief['uactAffirm'] = 0
            belief['uactInform'] = 0
            belief['uactRequest'] = 0
            
            if track['userAct']:
                if 'CONFIRM' in track['userAct']:
                    belief['uactConfirm'] = 1
                if 'AFFIRM' in track['userAct']:
                    belief['uactAffirm'] = 1
                if 'INFORM' in track['userAct'] or 'INFORM_INTENT' in track['userAct']:
                    belief['uactInform'] = 1
                if 'REQUEST' in track['userAct']:
                    belief['uactRequest'] = 1
            
            belief['sysActs'] = dict.fromkeys(self.summaryaction.action_names, 0.0)

            if track['sysAct']:
                sumAct = track['sysAct'].split('(')[0]
                sumSlot = track['sysAct'].split('(')[1].split(')')[0]
                if '=' in sumSlot:
                    sumSlot = sumSlot.split('=')[0]
                if sumSlot and sumAct not in ["inform", "inform_byname", "inform_alternatives", "bye", "repeat", "reqmore", "restart", "notify", "offer"]:
                    summaryAct = sumAct + '_' + sumSlot
                else:
                    summaryAct = sumAct

                if summaryAct in belief['sysActs']:
                    belief['sysActs'][summaryAct] += 1
