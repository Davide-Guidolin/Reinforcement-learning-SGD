...
def getInformNoneVenue(constraints, domainString):
    '''
    ...
    if "name" not in feats:
        feats["name"] = Ontology.global_ontology.getRandomValueForSlot(domainString, 'name', nodontcare=True)
...
def getInformByConstraints(constraints, domainString, lastInformedVenue):
    ...
    if len(entities) == 0:
        return getInformNoneVenue(constraints, domainString)
    else:
        ret_ent = entities[0]
        for ent in entities:
...
def getInformRequestedSlots(requested_slots, name, domainString):
    ...
    if len(result) > 0:
        ...
        if not name:
            ...
            return getInformNoneVenue({'name': name}, domainString)

def getOfferNoneVenue(constraints):
    feats = {}
    for slot in constraints:
        if constraints[slot] != 'dontcare':
            feats[slot] = constraints[slot]
    return 'offer(name=none, {})'.format(convertFeatsToStr(feats))

def getOfferEntity(accepted_values, ent):
    '''
    Converts a database entity into a dialogue act
    ...
    return 'offer({})'.format(convertFeatsToStr(feats))

def getOfferByConstraints(constraints, domainString, lastInformedVenue):
    entities = Ontology.global_ontology.entity_by_features(domainString, constraints)
    if len(entities) == 0:
        return getOfferNoneVenue(constraints)
    else:
        ret_ent = entities[0]
        for ent in entities:
            if ent['name'] == lastInformedVenue:
                ret_ent = ent
                break
        return getOfferEntity(constraints, ret_ent)

def getOfferRequestedSlots(requested_slots, domainString):
    result = []
    while len(result) == 0:
        rand_name = Ontology.global_ontology.getRandomValueForSlot(domainString, 'name', nodontcare=True)
        result = Ontology.global_ontology.entity_by_features(domainString, {'name': rand_name})
    ent = result[0]
    return _getOfferRequestedSlotsForEntity(requested_slots, ent, domainString)


def _getOfferRequestedSlotsForEntity(requested_slots, ent, domainString):
    slot_list = _getInformRequestedSlotsForEntity(requested_slots, ent, domainString).split("(")[1].split(")")[0]
    return 'offer({})'.format(slot_list)
