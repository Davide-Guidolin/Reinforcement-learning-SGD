...
def plotTrain(dname, rtab, stab, title, block=True, saveplot=False):
    font = {
            'weight': 'bold',
            'size': 20}
    plt.rc('font', **font)
    ...
    for policy in policylist:
        tab = rtab[policy]
        plt.subplot(211)
        plt.xlim((0, len(tab['x']+5)))
        if len(tab['x']) < 2:
            plt.axhline(y=tab['y'][0], linestyle='--')
        else:
            tab['x'] = np.array([i for i in range(1, len(tab['x'])+1)])
            plt.errorbar(tab['x'],tab['y'], yerr=tab['var'], label=policy)
            plt.xticks(np.arange(min(tab['x']), max(tab['x'])+5, step=max(1, (max(tab['x'])+5)/50)), fontsize=10)
...
def plotTrainLogs(logfilelist, printtab, noplot, saveplot, datasetname, title, block=True):
    """
    ...
    try:
        ...
        if ncurves > 0:
            ...
            for domain in domains:
                ...
                if not noplot:
                    plotTrain(datasetname+'-'+domain,rtab,stab,block=block,saveplot=saveplot, title=title)
            # Print average for all domains
            if len(domains) > 1:
                av_rtab, av_stab, av_ttab = getAverageResults(average_results)
                plotTrain(datasetname+'-mean', av_rtab, av_stab, block=block,saveplot=saveplot, title=title)
        else:
...
def plot_command(args="", printtab=False, noplot=False, saveplot=False, datasetname='', title='Performances'):
    ...
    if trainlogs:
        plotTrainLogs(trainlogs, printtab, noplot, saveplot, datasetname, title, block)
    if testlogs:
...
def evaluate_actions_command(configfile, test_file, seed=None, trainerrorrate=None, trainsourceiteration=None):
    configId = getConfigId(configfile)
    if seed:
        seed = int(seed)
    initialise(configId, configfile, seed, "chat", trainerrorrate=trainerrorrate,
    ...
    evaluator = ActionEvaluator.ActionEvaluator(trainerrorrate)
    evaluator.run_eval(test_file)
