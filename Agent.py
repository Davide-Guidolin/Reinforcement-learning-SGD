...
class DialogueAgent(object):
    ...
    def continue_call(self, asr_info, domainString=None, domainSimulatedUsers=None, supervised=False, real_sys_act=None):
        '''
        ...
        # EVALUATION: - record the system action taken in the current domain if using tasks for evaluation (ie DialogueServer)
        self._evaluate_agents_turn(domainSimulatedUsers, sys_act, state, supervised, real_sys_act)
...
#         logger.info('agent: _hand_control')
    ...
    def _evaluate_agents_turn(self, domainSimulatedUsers=None, sys_act=None, state=None, supervised=False, real_sys_act=None):
        '''  
...
#                 self.evaluation_manager.domainEvaluators[operatingDomain].rnn.set_turn_feature(belief,prev_sys_act,turnNo)
        ...
        turnInfo['supervised'] = supervised
        turnInfo['real_sys_act'] = real_sys_act
